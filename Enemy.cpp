	...
	:Base(), segment_(segment), isAlive_(true)
{
	...
	Color_ = GetColor(255, 255, 255);//白 ←色で点数分けてみるのも面白いかも
	if (segment_ < SEGMENT_NUM)
		segment_ = SEGMENT_NUM;//最低8分割
	radius_ = (float)(GetRand(50) + MIN_RADIUS);//25から75の間でランダムな半径

	size_ = checkSize();//大中小のサイズ情報取得

	vertex_.resize(segment_);//頂点配列のサイズ変更
	angle_ = 0.0f;
	omega_ = (float)(GetRand((int)(MAX_OMEGA * 100))) / 100.0f;//0から2の間でランダムな角速度
	if (GetRand(2) == 0) omega_ = -omega_;//正負ランダム
	MakeShape();
...
void Enemy::Update()
{
	if (IsAlive() == false)return;//死んでるならスルー
